...
import {
  ...
  hideMessage,
  setstatustoinitial,
} from "./../../../../appRedux/actions/ExpdocumentsActions";
import IntlMessages from "util/IntlMessages";

let userId = "";

class ExpDocuments extends React.Component {
  get_expdocumentsById(pageNumber = "", visit_id = "") {
    if (
      this.props.status === "Initial" ||
      (pageNumber === "" && visit_id !== "")
    ) {
      this.props.get_expdocuments({ pageNumber: 1, visit_id: visit_id });
    } else {
      this.props.get_expdocuments({
  ...
  componentDidUpdate(prevProps) {
    if (!prevProps.open) {
      if (
        ...
        this.get_expdocumentsById(1, this.props.visit_id);
      }
    }
  }
  ...
  handleTableChange = (pagination) => {
    ...
    this.get_expdocumentsById(pagination.current, this.props.visit_id);
  };
  ...
  render() {
    ...
    var ExpdocumentsData = this.props.getExpdocumentsData;
    var ExpdocumentData = "";
    ...
    if (!ExpdocumentsData) {
      ...
      // Object is NOT empty
      ExpdocumentData = ExpdocumentsData.DocumentList;

      const pagination = { ...this.state.pagination };

      pagination.total = ExpdocumentsData.TotalCount;
...
const mapStateToProps = (state) => {
  return {
    getExpdocumentsData: state.expdocumentsReducers.get_expdocuments_res,
    loader: state.expdocumentsReducers.loader,
    showSuccessMessage: state.expdocumentsReducers.showSuccessMessage,
    successMessage: state.expdocumentsReducers.successMessage,
    //authUser : state.auth.authUser,
    showMessage: state.expdocumentsReducers.showMessage,
    alertMessage: state.expdocumentsReducers.alertMessage,
    status: state.expdocumentsReducers.status,
  };
};
export default connect(mapStateToProps, mapDispatchToProps)(ExpDocuments);
